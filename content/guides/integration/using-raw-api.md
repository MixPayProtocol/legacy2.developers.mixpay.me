...
1. In this view:
    ...
    2. Using [Payment Assets ](https://developers.mixpay.me/api/assets/payment-assets)API to let customers select which cryptocurrency they want to pay;
...
2. When the customer **clicks to confirm**, based on which wallet has been chosen: 
    1. If the customer chooses "Mixin Wallet", you can call our [accepting-payments API](https://developers.mixpay.me/api/payments/accepting-payments) to create a payment;
    
    2. If It's an "On-chain Wallet", should call our [on-chain-payments API](https://developers.mixpay.me/api/payments/onchain-payments) to create a payment;
...
In step 3, if the customer is selecting Pay using Mixin Wallet, your App should call accepting-payments API](https://developers.mixpay.me/api/payments/accepting-payments).
...
In step 3, if the customer is selecting Pay using the on-chain Wallet, your App should call on-chain payment API](https://developers.mixpay.me/api/payments/onchain-payments).

> Note: [on-chain-payments API](https://developers.mixpay.me/api/payments/onchain-payments) and [accepting-payments API](https://developers.mixpay.me/api/payments/accepting-payments) use the same endpoint; the only difference is the on-chain payment API payload with an `isChain` key set to `true`.
