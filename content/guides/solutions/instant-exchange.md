...
-   [Payment Assets](https://developers.mixpay.me/api/assets/payment-assets) - Used to let payers choose which payment cryptocurrency they want to pay;
...
-   [Settlement Assets](https://developers.mixpay.me/api/assets/settlement-assets) - Used when you are setting the payee settlement type.
...
3.  Also, if the user UUID you want to acquire is a MixPay user, you can get this UUID via [getting Mixin UUID API](https://developers.mixpay.me/api/users/get-mixin-uuid).
...
Finally, use the [payments-results API](https://developers.mixpay.me/api/payments/payments-results) to loop query payments result; the parameter is the `traceId` you generated. And the response is the `status` of payment. The `status` has three parameters, unpaid failed and succeeded.
